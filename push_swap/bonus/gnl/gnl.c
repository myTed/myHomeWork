...
/*   Updated: 2022/07/15 19:26:33 by kyolee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
/*
#include <stdio.h>
...
int	save_line(int fd, char **pline, t_list_gnl **pplist)
{
	...
	if (*pline == 0)
		return (0);
	len = ft_strlen_gnl(*pline);
	if (is_found_chr(*pline, '\n', len - 1) == 0)
		return (0);
	enter_pos = 0;
	while ((*pline)[enter_pos] != '\n')
		enter_pos++;
	tmp = ft_strjoin_gnl(&(*pline)[enter_pos + 1], 0, len - enter_pos - 1, 0);
	if (tmp == 0)
		return (-1);
	new_lst = ft_lstnew_gnl(fd, tmp, len - enter_pos - 1);
	if (new_lst == 0)
	{
		free(tmp);
		return (-1);

...
int	local_to_world_cord(t_draw_info *pdi)
{
	int	idx;
	...
	while (idx < pdi->pmap->width * pdi->pmap->height)
	{
		pdi->pvcord[idx].x = pdi->pvcord[idx].y;
		pdi->pvcord[idx].y = pdi->pvcord[idx].x;
		pdi->pvcord[idx].z = -pdi->pvcord[idx].z;
		idx++;
	}
	return (0);
}
...
int	front_view(t_draw_info *pdi)
{
	int	idx;
	...
	{
		pdi->pvcord[idx].x = pdi->pvcord[idx].y;
		pdi->pvcord[idx].y = -pdi->pvcord[idx].z;
		pdi->pvcord[idx].z = -pdi->pvcord[idx].x;
		idx++;
	}
	return (0);
}

int	right_view(t_draw_info *pdi)
{
	int	idx;
	...
	while (idx < pdi->pmap->width * pdi->pmap->height)
	{
		pdi->pvcord[idx].x = -pdi->pvcord[idx].x;
		pdi->pvcord[idx].y = -pdi->pvcord[idx].z;
		pdi->pvcord[idx].z = -pdi->pvcord[idx].y;
		idx++;	
	}
	return (0);
}

int	top_view(t_draw_info *pdi)
{
	int	idx;
	int	tmp;
	...
	while (idx < pdi->pmap->width * pdi->pmap->height)
	{
		tmp = pdi->pvcord[idx].x;
		pdi->pvcord[idx].x = pdi->pvcord[idx].y;
		pdi->pvcord[idx].y = tmp;
		pdi->pvcord[idx].z = -pdi->pvcord[idx].z;
		idx++;
	}
	return (0);
...
int iso_metric2(t_draw_info *pdi, int tx, int ty, int tz)
{
	double	tmp_x;
	double	tmp_y;
	double	tmp_z;
	...
	while (idx < pdi->pmap->width * pdi->pmap->height)
	{
		...
		pdi->pvcord[idx].y = (0.40824436 * tmp_x) +(0.40824436 * tmp_y) + (0.8165 * tmp_z);
		pdi->pvcord[idx].z = (0.5777350 *tmp_x) + (0.577350 * tmp_y) + (-0.577350 * tmp_z);
		*/
			
		pdi->pvcord[idx].x = (tmp_x * -0.70710) + (0.70710 * tmp_y) + 
			(-tx * (tmp_x * -0.70710)) + (-ty * (0.70710 * tmp_y));
		pdi->pvcord[idx].y = (0.408244 * tmp_x) +(0.408244 * tmp_y) + (-0.8165 * tmp_z) +
			(-tx * (tmp_x * 0.408244)) + (-ty * (0.408244 *tmp_y)) + (-tz * (-0.8165 * tmp_z));
		pdi->pvcord[idx].z = (-0.57735 *tmp_x) + (-0.57735 * tmp_y) + (-0.577344 * tmp_z)
				+ (-tx * (tmp_x * -0.57735)) + (-ty * (-0.577344 * tmp_y)) + (-tz * (-0.577344 * tmp_z));
		
		/*	
		pdi->pvcord[idx].x = (0.57735 * tmp_x) + (-0.7071 * tmp_y) + (0.408244 * tmp_z) +
			(-tx * (tmp_x * -0.70710)) + (-ty * (0.70710 * tmp_y));
		pdi->pvcord[idx].y = (0.57735 * tmp_x) +(0.70710 * tmp_y) + (0.408244 * tmp_z) + 
			(-tx * (tmp_x * 0.408244)) + (-ty * (0.408244 *tmp_y)) + (-tz * (-0.8165 * tmp_z));
		pdi->pvcord[idx].z = (-0.577344 *tmp_x)  + (-0.8165 * tmp_z)
				+ (-tx * (tmp_x * -0.57735)) + (-ty * (-0.577344 * tmp_y)) + (-tz * (-0.577344 * tmp_z));
		*/
		idx++;
	}
...
int	draw_screen(t_draw_info *pdi)
{
	...
	while (idx < pdi->pmap->width * pdi->pmap->height)
	{
		pdi->pvcord[idx].x *= SCREEN_DX;
		pdi->pvcord[idx].y *= SCREEN_DY;
		idx++;
	}
	idx = 0;
	while (idx < pdi->pmap->width * pdi->pmap->height)
...
int	key_event(int keycode, void *param)
{
	...
	if (keycode == END_ROL_X)
	{
		...
		make_matrix_rotate_x(&tmp, -10);
		multiply_matrix(pdi->protate_m, &tmp, pdi->protate_m);
		make_final_matrix(pdi);
...
int	main(int argc, char *argv[])
{
	...
	draw_info.pmlx = &min;
	draw_info.scale = 1.0;
	draw_info.protate_m = &rotate_m;
	draw_info.piso_m = &iso;
	draw_info.pview_m = &view;

	init_matrix(&rotate_m);
	init_matrix(&iso);
	init_matrix(&view);
